...
// Deliberately the same shape, spacing and hover choreography as BookCard so
// the two shelves read as one Bookverse. Only the badges differ: a community
// book shows who contributed it instead of a rating.
const UsedBookCard = ({ book, index = 0 }) => {
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-navy-950/70 via-navy-950/0 to-navy-950/0 opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
        ...
        <span
          className={`absolute left-3 top-3 rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wide backdrop-blur-md transition-all duration-300 group-hover:scale-105 ${
            book.available ? "bg-emerald-500/90 text-white" : "bg-navy-900/80 text-cream-100"
          }`}
        >
